...
describe('audio module e2e (real Whisper + LanceDB)', () => {
  ...
  it(
    'add_audio_file → search_audio finds the transcript',
    async () => {
      ...
      const addRes = (await tools.find((t) => t.name === 'add_audio_file')!.handler({
        path: TEST_AUDIO,
      })) as { id: string; duration: number };
      expect(addRes.id).toBeDefined();
      expect(addRes.duration).toBeGreaterThan(0);

      const hits = (await tools.find((t) => t.name === 'search_audio')!.handler({
        query: 'hello test',
      })) as Array<{ id: string }>;
