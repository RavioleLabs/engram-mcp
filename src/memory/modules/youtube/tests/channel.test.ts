...
describe('resolveChannelId', () => {
  ...
  it('resolves a channel URL to a channel ID', async () => {
    let id: string;
    try {
...
describe('RSS feed fetch (real network)', () => {
  it('fetches and parses the channel RSS feed', async () => {
    ...
    try {
      ...
      console.warn(
        `Skipping RSS test — network error: ${e instanceof Error ? e.message : String(e)}`,
      );
      return;
    }
    if (!res.ok) {
